...
// Images
let imagemin;
import('gulp-imagemin').then((module) => (imagemin = module.default));
...
gulp.task('images:docs', async function () {
  ...
  await gulp.src('./src/img/**/*').pipe(changed('./docs/img/')).pipe(webp()).pipe(gulp.dest('./docs/img/'));

  return gulp
    .src('./src/img/**/*')
    .pipe(changed('./docs/img/'))
    .pipe(imagemin({ verbose: true })) // Исправление: используйте переменную imagemin
